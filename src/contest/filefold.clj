...
(defn- fold-channel [^FileChannel chn from to buffer-size combinef reducef]
  ...
  (defn read-buffer [from to]
    (let [buffer (ByteBuffer/allocate (- to from))]
      (.read chn buffer from)
      buffer))

  (defn reduce-buffer [^ByteBuffer buffer]
    (reduce reducef (combinef) (-> buffer (.array) (String. "UTF-8") (.split "[\n]"))))

  (if (<= (- to from) buffer-size)
    (reduce-buffer (read-buffer from to))
    (fjinvoke
     #(let [ffrom from
            fto (+ from buffer-size)
            bfrom (max fto (- to buffer-size))
            bto to
            tasks [(r/fjtask (reduce-buffer (read-buffer ffrom fto)))
                   (r/fjtask (reduce-buffer (read-buffer bfrom bto)))]]

       (doseq [task tasks] ))))

;    (let [tasks (keep identity [(front-task) (center-task) (back-task)])]
;
;      (fjinvoke
...
;ft-from from
;          ft-to (+ ft-from buffer-size)
;          bt-from (- end buffer-size)-to
;          bt-to end
